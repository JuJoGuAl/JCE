...
class database {
    ...
    public function __construct ($table, $key){
        ...
        */
        $this->rows_not_upper =array("clave","logo_path","foto_path","pdf_path","imagen_path");
        $this->fields = array ();
        $this->con_ = connect();
        if($this->con_["title"]=="SUCCESS"){
            $this->conn = $this->con_["content"];
            $this->produccion = $this->con_["pro"];
            $this->user = !isset($_SESSION['jce_log']) ? 'Template' : $_SESSION['jce_log'];
    ...
    */
    public function getRecords($campos_srt=false,$where_str=false, $group_str=false, $order_str=false,$limit_str=false,$having_str=false){
            ...
            if(array_key_exists(0, $where_str)){
                ...
                foreach ($where_str as $key => $value) {
                    ...
                    if(is_array($value["value"])){
                        ...
                        $values[]=$value["value"]==null ? $value["value"] : $value["value"];
                    }
                }
            }
        }
    ...
    */
    public function insertRecord($data){
        ...
        //$query = "SET @current_user = '{$this->user}'; SET @current_ip = '{$this->ip}'; ";
        $query = "";
    ...
    */
    public function upsertRecord($data){
        ...
        $campos_values =$this->getEditFields(true);
        //$query = "SET @current_user = '{$this->user}'; SET @current_ip = '{$this->ip}'; ";
        $query = "";
        ...
        foreach ($data as $key => $value) {
            ...
            if($value === 0 || $value === "0"){
                ...
                $values [] = NULL;
            }else if((in_array($campos_values[$key], $this->rows_not_upper))){
                $values [] = $value;
            }else{
                $values[] = $value;
            }
        ...
        array_shift($data);
        array_shift($campos_values);
        foreach ($data as $key => $value) {
            $query .= $campos_values[$key]." = ?, ";
            if($value===0 || $value==="0"){
                $values [] = 0;
            }else if ($value===""){
                $values [] = NULL;
    ...
    private function validateOperation($query,$values){
        if($this->con_["title"]=="SUCCESS"){
            ...
            try {
                ...
                $this->conn->exec('SET @current_ip = "' . $this->ip . '"');
